...
exports.optionsStorage = new NeDB({ filename: electron_1.app.getPath('userData') + '/storage/preferences', autoload: true });
...
function updateDisplayOptions(updatedOptions) {
    ...
    if (updatedOptions == null) {
        //Get saved options
        exports.optionsStorage.findOne({ _id: 'display' }, function (err, doc) {
        ...
        //Update options
        exports.optionsStorage.update({ _id: 'display' }, { _id: 'display', values: updatedOptions }, { upsert: true }, function (err, numReplaced, upsert) {
            displayOptions.display.list = getDisplayList();
            main_1.mainWindow.webContents.send('slides-options', displayOptions);
